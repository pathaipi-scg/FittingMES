...
## 12A. Production Quantity View

### `dbo.vw_DailyProductionQty`

Purpose: authoritative production quantity summary by production date and shift for USAGE calculations.
...
```text
ProductionLot.ProductionID = ProductionData.ProductionID
```

The view groups active production lots by:
...
| Column | Meaning |
|---|---|
| `ReportDate` | `ProductionLot.ProdDate` |
| `Shift` | saved production lot shift |
| `CounterQty` | sum of `ProductionData.CounterQty` for active lots in the date/shift |
| `CuringQty` | sum of `ProductionData.CuringQty` for active lots in the date/shift |
| `LotCount` | number of production lots included in the date/shift |

Current definition:

```sql
CREATE VIEW dbo.vw_DailyProductionQty
AS
SELECT
    L.ProdDate AS ReportDate,
    L.Shift,
    SUM(CAST(D.CounterQty AS bigint)) AS CounterQty,
    SUM(CAST(D.CuringQty  AS bigint)) AS CuringQty,
    COUNT_BIG(*) AS LotCount
FROM dbo.ProductionLot L
INNER JOIN dbo.ProductionData D
    ON D.ProductionID = L.ProductionID
WHERE L.IsActive = 1
GROUP BY
    L.ProdDate,
    L.Shift;
...
USAGE code must use this view for production denominators instead of independently reimplementing the ProductionLot/ProductionData aggregation.

---

## 12B. Shift Material Usage Calculation View

### `dbo.vw_DailyMaterialUsageCalc`

Purpose: calculated material usage for each date, shift, and material.
...
- `RawQty` = material usage quantity entered into MES.
- `CounterQty` = shift total from `vw_DailyProductionQty`.
- `CuringQty` = shift total from `vw_DailyProductionQty`.
- `LotCount` = number of active production lots in that shift.
- `QtyPer1000Counter`
- `QtyPer1000Curing`
...
```text
QtyPer1000Counter = RawQty * 1000 / CounterQty
QtyPer1000Curing  = RawQty * 1000 / CuringQty
```

The SQL view uses `NULLIF(..., 0)` to avoid division by zero.

The USAGE application must consume this view for calculated Shift 1 / Shift 2 results. Do not duplicate these formulas in another SQL query unless explicitly required.

---

## 12C. All-Day Material Usage Calculation View

### `dbo.vw_DailyMaterialUsageTotalCalc`

Purpose: calculated ALL DAY material usage by date and material.
...
dbo.vw_DailyMaterialUsageTotal
        +
daily aggregation of dbo.vw_DailyProductionQty
```

`vw_DailyMaterialUsageTotal` first sums raw material quantity across shifts for the same date/material.

`vw_DailyMaterialUsageTotalCalc` then sums production quantities across shifts for the same date and calculates:

```text
QtyPer1000Counter = TotalRawQty * 1000 / TotalCounterQty
QtyPer1000Curing  = TotalRawQty * 1000 / TotalCuringQty
```

This is the authoritative ALL DAY method.

Do **not** calculate ALL DAY as:

```text
Shift1 QtyPer1000 + Shift2 QtyPer1000
```

because that produces a different and incorrect normalization.

The USAGE application must consume this view for the ALL DAY calculated section rather than reimplementing the formula.

---

## 12D. USAGE SQL Data Flow

The intended SQL flow for the USAGE feature is:
...
DailyMaterialUsage
        |
        v
DailyMaterialUsageDetail
        |
        v
vw_DailyMaterialUsage
        |-----------------------------|
        v                             v
vw_DailyMaterialUsageCalc     vw_DailyMaterialUsageTotal
        ^                             |
        |                             v
vw_DailyProductionQty         vw_DailyMaterialUsageTotalCalc
        ^                             ^
        |                             |
ProductionLot + ProductionData--------|
```

Responsibilities are intentionally separated:

- `MaterialUsageMaster` defines dynamic material rows.
- `DailyMaterialUsage` / `DailyMaterialUsageDetail` store operator-entered raw usage.
- `vw_DailyMaterialUsage` provides shift/detail usage with master display information.
- `vw_DailyMaterialUsageTotal` sums raw material usage across the day.
- `vw_DailyProductionQty` supplies production Counter/Curing denominators.
- `vw_DailyMaterialUsageCalc` supplies shift-level normalized results.
- `vw_DailyMaterialUsageTotalCalc` supplies all-day normalized results.

**Codex rule:** reuse these views. Do not create duplicate calculation views, fixed material columns, or independent Counter/Curing aggregation without explicit approval.
